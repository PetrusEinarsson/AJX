...
#### TODO: Check dangerous ChatGPT code below!!!!!
def hat(v):
    """3-vector -> 3x3 skew-symmetric matrix."""
    x, y, z = v
    return jnp.array([[0.0, -z, y], [z, 0.0, -x], [-y, x, 0.0]], dtype=v.dtype)


@jax.jit
def Jr_SO3_inv_extT(phi, small_angle_thresh=1e-6):
    """
    Inverse right Jacobian of SO(3).

    Args:
        phi: (3,) rotation vector
        small_angle_thresh: threshold for small-angle series expansion

    Returns:
        3x3 matrix J_r^{-1}(phi)
    """
    theta = jnp.linalg.norm(phi)
    X = hat(phi)
    X2 = X @ X
    I3 = jnp.eye(3, dtype=phi.dtype)

    def small_angle(_):
        # Series expansion: I + 1/2 X + 1/12 X^2
        return I3 + 0.5 * X + (1.0 / 12.0) * X2

    def large_angle(_):
        coeff = (1.0 / (theta * theta)) - (1.0 + jnp.cos(theta)) / (
            2.0 * theta * jnp.sin(theta)
        )
        return I3 + 0.5 * X + coeff * X2

    res = jax.lax.cond(
        theta < small_angle_thresh, small_angle, large_angle, operand=None
    )
    return jax.scipy.linalg.block_diag(I3, res.T)
